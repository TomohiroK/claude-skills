...
## 実在を確認していないものを書かない（最重要）
- URL・ドメイン・製品名・外部サービスの挙動は、確認してからでなければ書けない
- ドメインはコードに直書きせず、環境変数から受け取る
- DNS が解決することは「存在する」の証拠にならない
- 詳細: `.claude/rules/no-fabrication.md`

## 検証は「壊れる場所」を通っているか（最重要）
- 手元で通ることは、動く証拠ではない
- 本番で落ちるのに再現しないときは、まず「壊れている部品が手元に存在するか」を疑う
- 「動きます」ではなく「どの環境で、何を通して、何を通していないか」を報告する
- 直したら、その経路を検証項目に足す
- 詳細: `.claude/rules/verification-scope.md`

## 見えるところで作業する
- 手を動かす前に何をするかを言う。出力は要約せず生で貼る。差分は実物を出す
- 1つ終えたら報告して止まる。勝手に次へ進まない
- 詳細: `.claude/rules/visible-work.md`
