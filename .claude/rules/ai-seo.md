...
4. **動的ルート競合の確認**: `[slug]` 等の動的ルートがある場合、`public/llms*.txt` が遮蔽されないか確認する。競合する場合は `next.config.js` の `rewrites({ beforeFiles })` で明示的にルーティングする
5. ローカルで全ファイルの200 OK + linkタグ出力を検証
6. デプロイ後に本番で同様の検証を実施
