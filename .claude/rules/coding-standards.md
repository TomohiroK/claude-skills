...
### デバッグログのライフサイクル
- エラー調査で追加したデバッグログ（NSLog, console.log, print等）は、問題解決後も**削除しない**
- プロジェクトのクローズ時に全デバッグログを**一括削除**する
- 一度起きた問題は再現する可能性がある。ログを消すと再発時にまた仕込み直す手間が発生する
- 削除タイミング: プロジェクト完了・納品・クローズの直前に一括で実施
